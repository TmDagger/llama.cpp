...
struct common_params {
    ...
    std::vector<int32_t> expert_cache_slots_per_dev; // per-device slot counts (empty = broadcast expert_cache_slots)
    std::vector<int32_t> expert_cache_rail_mb = std::vector<int32_t>(llama_max_devices(), 768); // per-device VRAM reserve (MiB)
    ...
    int32_t expert_cache_external_reserve_dev = -1; // device index to hold the whole reserve on; -1 = spread
    int64_t draft_reserve_bytes = 0;                // bytes kept free on the last device when distributing layers (draft pinned there)
